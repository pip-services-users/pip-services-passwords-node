...
class EmailConnector {
    ...
    sendEmail(correlationId, userId, message, parameters) {
        if (this._emailClient == null)
            return;
        let recipient = new pip_clients_email_node_1.RecipientV1(userId);
    ...
    sendAccountLockedEmail(correlationId, userId) {
        let message = new pip_clients_email_node_2.MessageV1(null, null, null, this._accountLockedSubject, this._accountLockedText, this._accountLockedHtml);
        this.sendEmail(correlationId, userId, message, null);
    }
    sendPasswordChangedEmail(correlationId, userId) {
        let message = new pip_clients_email_node_2.MessageV1(null, null, null, this._passwordChangedSubject, this._passwordChangedText, this._passwordChangedHtml);
        this.sendEmail(correlationId, userId, message, null);
    }
    sendRecoverPasswordEmail(correlationId, userId) {
        let message = new pip_clients_email_node_2.MessageV1(null, null, null, this._recoverPasswordSubject, this._recoverPasswordText, this._recoverPasswordHtml);
        this.sendEmail(correlationId, userId, message, null);
    }
}
